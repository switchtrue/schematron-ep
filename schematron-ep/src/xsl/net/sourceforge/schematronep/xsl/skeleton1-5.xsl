...
<xsl:template name="process-message">
   ...
   <xsl:param name="role" />
   <axsl:variable name="message">
   		|<axsl:value-of select="saxon:line-number(.)"/>:<xsl:apply-templates mode="text"/> (<xsl:value-of select="$pattern" /><xsl:if test="$role"> / <xsl:value-of select="$role" /></xsl:if>)
   </axsl:variable>
   <axsl:value-of select="normalize-space($message)"/><axsl:text>&#13;&#10;</axsl:text>
</xsl:template>
<!-- End of Schematron message -->
